...
class Transaction extends React.Component {
    constructor(props) {
        ...
        this.processCancelTransactoin = this.processCancelTransactoin.bind(
            this
        );
        this.processCancelTransactoinError = this.processCancelTransactoinError.bind(
            this
        );
    ...
    getTransactionData() {
        ...
        axios
            .get(
                `${ROOT_URL}/api/transaction/check/` +
    ...
    postCancelTransaction(e) {
        ...
        axios
            .get(
                `${ROOT_URL}/api/transaction/delete/` +
                    localStorage.getItem('userName') +
                    '/' +
                    localStorage.getItem('password') +
                    '/' +
            ...
            this.props.transactionDataProp.forEach(function(transaction) {
                if (typeof transaction.goSegments != 'undefined') {
                    ...
                    transaction.goSegments.forEach(function(goSegment, i) {
                        if (i === 0) {
                            ...
                            goRows.push(<td>{goSegment.trainName}</td>);
                            goRows.push(
                                <td>
                                    {goSegment.fast === true ? 'Yes' : 'No'}
                                </td>
                            );
                            goRows.push(<td>{goSegment.startDay}</td>);
                            goRows.push(<td>{goSegment.startTime}</td>);
                            goRows.push(<td>{goSegment.endDay}</td>);
                            goRows.push(<td>{goSegment.endTime}</td>);
                            goRows.push(<td>{goSegment.startStation}</td>);
                            goRows.push(<td>{goSegment.endStation}</td>);
                            goRows.push(<td>{goSegment.price}</td>);
                            goRows.push(<td>{goSegment.passengers}</td>);
                        } else {
                            goRowsCont.push(
                                <tr>
                                    <td />
                ...
                if (typeof transaction.backSegments != 'undefined') {
                    ...
                    transaction.backSegments.forEach(function(backSegment, i) {
                        if (i === 0) {
                            backRows.push(<td>{backSegment.trainName}</td>);
                            backRows.push(
                                <td>
                                    {backSegment.fast === true ? 'Yes' : 'No'}
                                </td>
                            );
                            backRows.push(<td>{backSegment.startDay}</td>);
                            backRows.push(<td>{backSegment.startTime}</td>);
                            backRows.push(<td>{backSegment.endDay}</td>);
                            backRows.push(<td>{backSegment.endTime}</td>);
                            backRows.push(<td>{backSegment.startStation}</td>);
                            backRows.push(<td>{backSegment.endStation}</td>);
                            backRows.push(<td>{backSegment.price}</td>);
                            backRows.push(<td>{backSegment.passengers}</td>);
                        } else {
                            backRowsCont.push(
                                <tr>
                                    <td />
                        ...
                        <td>
                            {expired === true ? (
                                ...
                                <Button
                                    ...
                                    onClick={
                                        this.props.postCancelTransactionProp
                                    }
                                >
                                    Cancel
                                </Button>
